...
Background::Background() {
	_backgroundColor = NIL_COLOR;
	_iconColor = WHITE;
	_foregroundColor = NIL_COLOR;
	_speed = 0.5;
	_position = 0;
	_texture = nullptr;
	_imgWidth = 0;
...
Background::Background(vita2d_texture* texture, unsigned int backgroundColor, unsigned int iconColor) {
	_texture = texture;
...
Background::Background(vita2d_texture* texture, unsigned int backgroundColor, unsigned int iconColor, unsigned int foregroundColor, float speed) {
	_texture = texture;
	_imgWidth = vita2d_texture_get_width(texture);
	_imgHeight = vita2d_texture_get_height(texture);
	_backgroundColor = backgroundColor;
...
void Background::setBackgroundColor(unsigned int backgroundColor){
	_backgroundColor = backgroundColor;
}

void Background::setIconColor(unsigned int iconColor){
	_iconColor = iconColor;
}

void Background::setForegroundColor(unsigned int foregroundColor){
	_foregroundColor = foregroundColor;
}
